(function () {
  ...
  function defaults() {
    return {
      ...
      board: {
        ...
        done: ["Choose the shared Aero palette"]
      },
      settings: {
        density: "comfortable",
        autoplay: true,
  ...
  function renderReels() {
    ...
    root.innerHTML = '<div class="reels-layout"><aside class="reels-note"><span class="eyebrow">YOUR CONTROL</span><h2>Instant stops when you do.</h2><p>No infinite autoplay trap. Move deliberately, turn sound on for each Instant, or save the useful part to Workspace.</p><div class="reel-key"><span>↑ ↓</span> move between Instants</div></aside>' +
      ...
      }).join("") + '</div><aside class="reels-context"><div><span class="eyebrow">AFTER WATCHING</span><h3>Keep the useful next step.</h3><p>Saved Instants appear in Workspace. You can also create a private action in Aero without copying the public conversation.</p><a href="./">Open Aero ↗</a></div></aside></div>';
  }
  ...
  function renderChannels() {
    ...
    root.innerHTML = '<div class="channels-shell"><aside class="channel-rail"><div class="channel-workspace"><img src="../assets/connect_logo.svg" alt=""><div><strong>Connect studio</strong><small>' + allChannels().length + ' channels · ' + (huddle ? "huddle live" : "all quiet") + '</small></div></div><div class="channel-section-title"><span>Channels</span><button type="button" data-suite-action="new-channel">+</button></div><nav>' + allChannels().map(function (item) {
  ...
  function renderWorkspace() {
    ...
    if (suite.workspaceTab === "pages") {
      ...
      html += '<div class="workspace-calendar"><div class="calendar-days">' + ["MON 10", "TUE 11", "WED 12", "THU 13", "FRI 14"].map(function (day, index) { return '<section><strong>' + day + '</strong>' + (index === 1 ? '<article class="cal-blue"><span>10:00</span><b>Prototype review</b><small>#design-critique</small></article>' : '') + (index === 3 ? '<article class="cal-green"><span>16:00</span><b>Research huddle</b><small>#research</small></article>' : '') + (index === 4 ? '<article class="cal-coral"><span>18:30</span><b>Critique without theatre</b><small>Connect event</small></article>' : '') + '</section>'; }).join("") + '</div><a class="lyfe-calendar-link" href="./">Open your private schedule in Aero ↗</a></div>';
    } else {
      ...
      html += '<div class="workspace-saved"><div class="saved-summary"><span><b>' + suite.savedReels.length + '</b> Instants</span><span><b>' + suite.savedOpportunities.length + '</b> opportunities</span><span><b>' + suite.pinnedMessages.length + '</b> pinned messages</span><a href="./#library">Open private Library in Aero ↗</a></div><div class="saved-grid">' + savedReels.map(function (reel) { return '<article><span>INSTANT</span><h3>' + esc(reel.title) + '</h3><p>' + esc(reel.creator) + '</p><button type="button" data-suite-action="save-to-lyfe" data-kind="reel" data-id="' + esc(reel.id) + '">Save to Aero Library</button></article>'; }).join("") + savedOpps.map(function (item) { return '<article><span>OPPORTUNITY</span><h3>' + esc(item.title) + '</h3><p>' + esc(item.org) + '</p><button type="button" data-suite-action="save-to-lyfe" data-kind="opportunity" data-id="' + esc(item.id) + '">Save to Aero Library</button></article>'; }).join("") + (!savedReels.length && !savedOpps.length ? '<div class="saved-empty"><h3>Useful things can wait here.</h3><p>Save an Instant, opportunity, post, file, or message without turning it into a task.</p></div>' : '') + '</div></div>';
    }
    root.innerHTML = html;
  ...
  function showAuth() {
    if (authMode === "cloud" && window.LyfeCloud && LyfeCloud.user) {
      ...
      modal("Your Aero account", "One identity for Aero and Connect, with separate private and shared data spaces.", '<div class="account-panel"><span class="account-avatar">' + esc((user.name || user.email || "Y").slice(0, 2).toUpperCase()) + '</span><div><strong>' + esc(user.name || "Your account") + '</strong><small>' + esc(user.email || "Signed in with Google") + '</small></div></div><div class="account-separation"><article><span>AERO</span><p>Private tasks, notes, goals, tracking, Library, Gmail saves, and personal context.</p></article><article><span>CONNECT</span><p>Profile, posts, Instants, network, messages, channels, and shared workspace.</p></article></div><div class="suite-form-actions"><button class="quiet-button" type="button" data-suite-action="sign-out">Sign out</button><button class="primary-button" type="button" data-suite-action="modal-close">Done</button></div>');
      return;
    }
  ...
  function renderAccountButton() {
    ...
    if (authMode === "cloud" && window.LyfeCloud && LyfeCloud.user) {
      ...
      button.setAttribute("aria-label", "Sign in to Connect");
    }
  }

  async function addToLyfe(kind, id) {
    if (!suite.settings.syncToLyfe) { notify("Enable Aero handoffs in Connect settings first."); return; }
    ...
    try {
      ...
      if (!lyfe || typeof lyfe !== "object") { notify("Open Aero once before saving to its Library."); return; }
      if (!Array.isArray(lyfe.saved)) lyfe.saved = [];
      if (!lyfe.saved.some(function (entry) { return entry.sourceId === id; })) {
        lyfe.saved.unshift({ id: uid(), source: "Connect", sourceId: id, kind: kind, title: item.title, body: item.caption || item.detail || "", savedAt: Date.now() });
      }
      ...
      notify("Saved to Aero Library.");
    } catch (error) { notify("Aero could not save that item on this device."); }
  }
  ...
  function shareItem(label) {
    var data = { title: "Connect", text: label || "Shared from Connect", url: location.href };
  ...
  document.addEventListener("click", function (event) {
    ...
    if (action === "sign-out") {
      if (window.LyfeCloud) LyfeCloud.signOut().finally(function () { authMode = "gate"; closeModal(); renderAccountButton(); notify("Signed out of Aero and Connect."); });
      return;
    }
    if (action === "home-feed") {
    ...
    if (action === "post-share") { shareItem("A post from Connect"); return; }
    ...
    if (action === "reel-share") { var reel = findReel(target.dataset.id); shareItem(reel ? reel.title : "An Instant from Connect"); return; }
    ...
    if (action === "opportunity-apply") { var opportunity = OPPORTUNITIES.filter(function (item) { return item.id === target.dataset.id; })[0]; if (!opportunity) return; modal(opportunity.title, opportunity.org + " · " + opportunity.place, '<div class="opportunity-detail"><p>' + esc(opportunity.detail) + '</p><div class="skill-row">' + opportunity.skills.map(function (skill) { return '<span>' + esc(skill) + '</span>'; }).join("") + '</div><h3>Before you apply</h3><p>Connect will use only your public profile and anything you explicitly add. Your private Aero data is never attached.</p><button class="primary-button" type="button" data-suite-action="application-save" data-id="' + esc(opportunity.id) + '">' + (has(suite.appliedOpportunities, opportunity.id) ? "Application saved" : "Save an application draft") + '</button></div>'); return; }
    ...
    if (action === "channel-canvas") { modal("#" + selectedChannel().name + " canvas", "The durable context beside the conversation.", '<div class="canvas-modal"><h3>Purpose</h3><p>' + esc(selectedChannel().purpose) + '</p><h3>Current decision</h3><p>Keep public Connect identity and private Aero context separate. Move only explicit saves and actions.</p><h3>Open questions</h3><ul><li>What belongs on a public profile?</li><li>Which notifications deserve to cross into Aero?</li><li>What evidence should accompany recommendations?</li></ul><button class="primary-button" type="button" data-suite-action="modal-close">Done</button></div>', true); return; }
