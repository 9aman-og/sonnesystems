...
(function () {
  ...
  function normalizeEpisodeStudy(value) {
    if (!value || typeof value !== "object" || Array.isArray(value)) return null;
    if (window.AeroEval && typeof window.AeroEval.normalizeStudy === "function") {
      return window.AeroEval.normalizeStudy(value);
    }
    ...
    var kind = ["action", "response", "proactive"].indexOf(value.kind) >= 0 ? value.kind : "action";
    var phase = ["baseline", "adapted", "holdout"].indexOf(value.phase) >= 0 ? value.phase : "baseline";
    var verification = ["environment", "user", "local", "synthetic", "none"].indexOf(value.verification) >= 0
    ...
    var expected = Math.max(0, Math.min(100, Math.round(Number(value.expectedConstraints || 0))));
    return {
      version: 1, taskKey: taskKey, kind: kind, phase: phase, verification: verification,
      ...
      satisfiedConstraints: Math.max(0, Math.min(expected, Math.round(Number(value.satisfiedConstraints || 0)))),
      followupWords: Math.max(0, Math.min(4_000, Math.round(Number(value.followupWords || 0)))),
      clarificationTurns: Math.max(0, Math.min(50, Math.round(Number(value.clarificationTurns || 0)))),
      correctionTurns: Math.max(0, Math.min(50, Math.round(Number(value.correctionTurns || 0)))),
      stalePreferenceViolation: value.stalePreferenceViolation === true,
      preferenceRevision: Math.max(0, Math.round(Number(value.preferenceRevision || 0))),
      synthetic: value.synthetic === true || verification === "synthetic",
    };
  }

  function normalizeStudyEffort(value) {
    if (!value || typeof value !== "object" || Array.isArray(value)) return null;
    return {
      followupWords: Math.max(0, Math.min(4_000, Math.round(Number(value.followupWords || 0)))),
      clarificationTurns: Math.max(0, Math.min(50, Math.round(Number(value.clarificationTurns || 0)))),
  ...
  function normalize(raw) {
    ...
    base.episodes = list(raw.episodes).filter(function (episode) {
      ...
      return Object.assign({}, episode, {
        ...
        study: normalizeEpisodeStudy(episode.study),
        studyEffort: normalizeStudyEffort(episode.studyEffort),
  ...
  function finishEpisode(aero, episodeId, outcome, metadata) {
    ...
    episode.study = normalizeEpisodeStudy(episode.study);
    episode.studyEffort = normalizeStudyEffort(episode.studyEffort);
    return aero;
  }
  ...
  function metrics(aero) {
    ...
    var directionalReady = comparable.length >= 5 && scored.length >= 10 && avgCold > avgComparable
      && intentAccuracyDelta != null && intentAccuracyDelta >= -0.02
      && (!promoted.length || falsePromotions.length / promoted.length <= 0.05);
    var longitudinal = window.AeroEval && typeof window.AeroEval.evaluate === "function"
      ? window.AeroEval.evaluate(window.AeroEval.fromEpisodes(aero.episodes)) : null;
    return {
      ...
      directionalReady: directionalReady,
      longitudinal: longitudinal,
      proofReady: !!(longitudinal && longitudinal.evidenceReady),
    };
  }
