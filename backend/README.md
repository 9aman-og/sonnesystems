...
| POST   | /auth/login     | Returns a bearer token (7-day default expiry)   |
...
- `app/config.py` reads bounded settings from environment variables (`SONNE_DB_PATH`,
  `SONNE_CORS_ORIGINS`, `SONNE_TRUSTED_HOSTS`, `SONNE_MAX_BODY_BYTES`,
  `SONNE_TOKEN_TTL_DAYS`, `SONNE_ENV`). Production defaults exclude local origins and hosts.
...
- `app/security.py`: PBKDF2-HMAC-SHA256 password hashing (600k iterations, per-user salt,
  and automatic work-factor upgrades on login);
...
- `app/middleware.py` is the HTTP boundary: trusted hosts, strict JSON content types,
  64 KiB request limits, per-client abuse throttles, request IDs, no-store behavior,
  and restrictive security headers.
- Routers are thin; anything with logic gets a test in `tests/`.
- Runtime and test dependencies are exactly pinned in `requirements.txt`.
...
Deploy behind managed TLS and request timeouts, run Uvicorn with `--no-server-header`,
and keep the application process and SQLite file on a least-privilege service account.
