...
function openLiveAero() {
  ...
  } catch { /* the Lyfe route still opens without the handoff */ }
  const allowed = ["today", "tracking", "library", "connect", "gmail", "profile"];
  const aeroFrom = allowed.includes(source) ? source : "today";
  location.href = `/lyfe/?aeroFrom=${encodeURIComponent(aeroFrom)}#/aero`;
}
...
function mayaFollowUp(query) {
  const body = `
    <div class="action-proposal">
      ...
      <div class="proposal-preview">I reviewed the launch checklist. The identity and interaction shell are ready; backend wiring and live Lyfe migration are still open. I’ll mark owners on those two next.</div>
