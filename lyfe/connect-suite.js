(function () {
  ...
  function renderReels() {
    ...
    root.innerHTML = '<div class="reels-layout"><aside class="reels-note"><span class="eyebrow">YOUR CONTROL</span><h2>Reels stop when you do.</h2><p>No infinite autoplay trap. Move deliberately, turn sound on per reel, or save the useful part to Workspace.</p><div class="reel-key"><span>↑ ↓</span> move between reels</div></aside>' +
      ...
      }).join("") + '</div><aside class="reels-context"><div><span class="eyebrow">AFTER WATCHING</span><h3>Keep the useful next step.</h3><p>Saved reels appear in Workspace. You can also create a private action in Lyfe without copying the public conversation.</p><a href="./">Open Lyfe ↗</a></div></aside></div>';
  }
  ...
  function renderWorkspace() {
    ...
    if (suite.workspaceTab === "pages") {
      ...
      html += '<div class="workspace-saved"><div class="saved-summary"><span><b>' + suite.savedReels.length + '</b> reels</span><span><b>' + suite.savedOpportunities.length + '</b> opportunities</span><span><b>' + suite.pinnedMessages.length + '</b> pinned messages</span><a href="./#library">Open private Library in Lyfe ↗</a></div><div class="saved-grid">' + savedReels.map(function (reel) { return '<article><span>REEL</span><h3>' + esc(reel.title) + '</h3><p>' + esc(reel.creator) + '</p><button type="button" data-suite-action="save-to-lyfe" data-kind="reel" data-id="' + esc(reel.id) + '">Save to Lyfe Library</button></article>'; }).join("") + savedOpps.map(function (item) { return '<article><span>OPPORTUNITY</span><h3>' + esc(item.title) + '</h3><p>' + esc(item.org) + '</p><button type="button" data-suite-action="save-to-lyfe" data-kind="opportunity" data-id="' + esc(item.id) + '">Save to Lyfe Library</button></article>'; }).join("") + (!savedReels.length && !savedOpps.length ? '<div class="saved-empty"><h3>Useful things can wait here.</h3><p>Save a reel, opportunity, post, file, or message without turning it into a task.</p></div>' : '') + '</div></div>';
    }
    root.innerHTML = html;
  ...
  function renderProfessionalProfile() {
    ...
    var content = "";
    if (suite.profileTab === "work") content = '<div class="profile-showcase"><article class="showcase-large"><span>FEATURED PROJECT</span><h3>' + esc(profile.prompt || "Share one concrete project so the right people know where to begin.") + '</h3><p>Pin a project, paper, reel, portfolio link, or open question here.</p><button type="button" data-action="new-post">Add featured work</button></article><article><span>OPEN TO</span><h3>' + esc(profile.intent || "Collaborators and useful conversations") + '</h3><p>Your availability is visible without exposing your private Lyfe plans.</p></article></div>';
  ...
  document.addEventListener("click", function (event) {
    ...
    if (action === "reel-share") { var reel = findReel(target.dataset.id); shareItem(reel ? reel.title : "A reel from Lyfe Connect"); return; }
    ...
    if (action === "follow") { var followId = target.dataset.id; if (!followId) { notify("That profile is not available."); return; } toggle(suite.following, followId); saveSuite(has(suite.following, followId) ? "Following." : "Unfollowed."); renderNetwork(); renderReels(); return; }
    if (action === "connect-person") {
      ...
      else { toggle(suite.connected, person.id); saveSuite("Connection saved. You can add a personal note before sending."); renderNetwork(); }
      return;
    }
  ...
  document.addEventListener("submit", function (event) {
    ...
    if (kind === "email-auth") {
      ...
      LyfeCloud.signInEmail(email).then(function () { form.innerHTML = '<div class="email-sent"><span>✓</span><h3>Check your email</h3><p>We sent a secure sign-in link to ' + esc(email) + '.</p></div>'; }).catch(function (error) { submit.disabled = false; notify(error.message || "The sign-in email could not be sent."); });
      ...
      closeModal(); renderReels(); notify("Reel added for this session.");
